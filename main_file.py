...
def init():
    # Define all pins
    globals.ONBOARD_LED_PIN = machine.Pin("LED", machine.Pin.OUT)
    ...
    # Connect WLAN
    color.change_color(globals.COLOR_WLAN_CONNECTING, fade_time=globals.FADE_DURATION)
    ...
    while not internet_connected:
        ...
        print("")
        color.change_color(globals.COLOR_WLAN_CONNECTING, fade_time=globals.FADE_DURATION, blink_freq=globals.BLINK_FREQUENCY)
...
def test_connectivity() -> bool:
    ...
    color.change_color(globals.COLOR_PINGING, fade_time=globals.FADE_DURATION)
...
def __main__():
    ...
    while True:
        ...
        if (response == None):
            ...
            color.change_color(globals.COLOR_API_FAIL, fade_time=globals.FADE_DURATION, blink_freq=globals.BLINK_FREQUENCY)
            ...
            try:
                ...
                if (response_json["open"] == "1"):
                    color.change_color(globals.COLOR_DOOR_OPEN, fade_time=globals.FADE_DURATION)
                    print("Door OPEN", end="")
                else:
                    color.change_color(globals.COLOR_DOOR_CLOSED, fade_time=globals.FADE_DURATION)
...
try:
    color.color_thread_init() # Needs to be called before starting thread, to ensure that the thread is not started before all the required variables have been defined
    print(globals.BLINK_FREQUENCY)
